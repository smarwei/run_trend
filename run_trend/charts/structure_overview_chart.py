...
class StructureOverviewChart(QWidget):
    ...
    def update_chart(self, aggregates: List[Dict[str, Any]], smoothing: str = 'off'):
        ...
        # Extract all metrics
        total_distances = [agg['total_distance_km'] for agg in complete_aggregates]
        num_runs = [agg['num_runs'] for agg in complete_aggregates]
        avg_distances = [agg['avg_distance_per_run_km'] for agg in complete_aggregates]
        longest_runs = [agg['longest_run_km'] for agg in complete_aggregates]
