...
class AvgDistanceChart(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Create chart
        self.chart = QChart()
        self.chart.setTitle(self.tr("Average Distance per Run"))
    ...
    def update_chart(self, aggregates: List[Dict[str, Any]], smoothing: str = 'off'):
        ...
        avg_distances = [agg['avg_distance_per_run_km'] for agg in complete_aggregates]

        # Apply smoothing if enabled
        if smoothing != 'off':
            avg_distances = Smoother.smooth_series(avg_distances, 'sma', smoothing)

        # Create series (smoothing already applied if enabled)
        series = QLineSeries()
        series.setName(self.tr("Avg Distance per Run"))

        for i, value in enumerate(avg_distances):
