...
class ActivityAggregator:
    ...
    def aggregate_by_week(activities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of weekly aggregates with is_complete flag
        """
        aggregates = ActivityAggregator._aggregate_by_period(activities, 'week')
        return ActivityAggregator.mark_incomplete_periods(aggregates, 'week')
    ...
    def aggregate_by_month(activities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of monthly aggregates with is_complete flag
        """
        aggregates = ActivityAggregator._aggregate_by_period(activities, 'month')
        return ActivityAggregator.mark_incomplete_periods(aggregates, 'month')
    ...
    def _compute_period_aggregate(
        ...
        # Duration metrics
        # 1. Average duration per run
        avg_duration_per_run_min = total_moving_time_min / num_runs if num_runs > 0 else 0.0
        avg_duration_per_run_h = total_moving_time_h / num_runs if num_runs > 0 else 0.0

        # 2. Longest duration
        longest_duration_s = max((a.get('moving_time', 0) for a in activities), default=0)
        longest_duration_min = longest_duration_s / 60.0
        longest_duration_h = longest_duration_s / 3600.0

        # 3. Average long run duration (top 30% by distance)
        if num_runs >= 3:
            sorted_by_distance = sorted(activities, key=lambda a: a.get('distance', 0), reverse=True)
            num_long_runs = max(1, int(num_runs * 0.3))
            long_runs = sorted_by_distance[:num_long_runs]
            total_long_run_time_s = sum(lr.get('moving_time', 0) for lr in long_runs)
            avg_long_run_duration_min = (total_long_run_time_s / 60.0) / num_long_runs
            avg_long_run_duration_h = (total_long_run_time_s / 3600.0) / num_long_runs
        else:
            avg_long_run_duration_min = 0.0
            avg_long_run_duration_h = 0.0
        ...
        return {
            ...
            # Duration metrics
            'avg_duration_per_run_min': avg_duration_per_run_min,
            'avg_duration_per_run_h': avg_duration_per_run_h,
            'longest_duration_min': longest_duration_min,
            'longest_duration_h': longest_duration_h,
            'avg_long_run_duration_min': avg_long_run_duration_min,
            'avg_long_run_duration_h': avg_long_run_duration_h
        }

    @staticmethod
    def is_period_complete(period_date: datetime, period_type: str) -> bool:
        """
        Check if a period is complete based on current date.

        Args:
            period_date: Start date of the period (Monday for week, 1st for month)
            period_type: 'week' or 'month'

        Returns:
            True if period is complete, False if still ongoing
        """
        now = datetime.now()

        if period_type == 'week':
            # Week is complete if current date is past Sunday of that week
            # period_date is Monday, so add 6 days to get Sunday
            sunday_of_period = period_date + timedelta(days=6)
            # Complete if we're past the end of that Sunday
            return now.date() > sunday_of_period.date()
        else:  # month
            # Month is complete if we're in a different month
            return (now.year > period_date.year or
                    (now.year == period_date.year and now.month > period_date.month))

    @staticmethod
    def mark_incomplete_periods(aggregates: List[Dict[str, Any]], period_type: str) -> List[Dict[str, Any]]:
        """
        Add is_complete flag to each aggregate.
        ...
        Returns:
            List of aggregates with is_complete field added
        """
        for aggregate in aggregates:
            period_date = aggregate.get('period_date')
            if period_date:
                aggregate['is_complete'] = ActivityAggregator.is_period_complete(
                    period_date,
                    period_type
                )
            else:
                # If no period_date, assume complete (backward compatibility)
